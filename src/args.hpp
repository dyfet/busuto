...
public:
    explicit bad_arg(const std::string& text) noexcept : std::runtime_error(text) {}
...
public:
    ...
    struct value_t {
        ...
        constexpr explicit value_t(bool f) : index(0), flag(f) {}
        constexpr explicit value_t(unsigned n) : index(1), num(n) {}
        constexpr explicit value_t(const char *s) : index(2), str(s) {}
    ...
    struct option final {
        friend class args;
        explicit option(char_p help) noexcept : value_(false), code_(0), counter_(false), usage_(nullptr), id_(nullptr), help_(help), next_(nullptr) {
            add_option();
        }

        option(char ch, char_p id, char_p help, bool counter = false) noexcept : value_(false), code_(ch), counter_(counter), usage_(nullptr), id_(id), help_(help), next_(nullptr) {
            add_option();
        }

        option(char ch, char_p id, char_p help, char_p usage) noexcept : value_(false), code_(ch), counter_(false), usage_(usage), id_(id), help_(help), next_(nullptr) {
            add_option();
        }
        ...
        constexpr auto value(long default_value = 0) const {
    ...
    constexpr static auto argv0() -> const auto& {
        return argv0_;
    }
    ...
    constexpr static auto list() -> const auto& {
        return argv_;
    }
