...
#if !defined(BUSUTO_CRYPTO_OPENSSL) && !defined(BUSUTO_CRYPTO_SODIUM) && !defined(BUSUTO_CRYPTO_WOLFSSL) && !defined(BUSUTO_CRYPTO_MINICRYPT)
#define BUSUTO_CRYPTO_OPENSSL
#endif

#include "binary.hpp"
#include "safe.hpp"

#ifdef BUSUTO_CRYPTO_OPENSSL
#define BUSUTO_CRYPTO_CONFIGURED
...
#ifdef BUSUTO_CRYPTO_SODIUM
#define BUSUTO_CRYPTO_CONFIGURED
#if defined(BUSUTO_CRYPTO_OPENSSL)
#error Can only define one kind of crypto
#endif
#include <sodium.h>
#endif

#ifdef BUSUTO_CRYPTO_MINICRYPT
#define BUSUTO_CRYPTO_CONFIGURED
...
public:
    ...
    void fill(Binary& buf) {
        ...
        if (len < 0 || len > std::numeric_limits<int>::max()) {
            throw std::runtime_error("Random fill sizw invalid");
        }

        auto *ptr = reinterpret_cast<std::uint8_t *>(buf.data());
        if (RAND_bytes(ptr, len) != 1) {
            throw std::runtime_error("Random fill failed");
        }
    };
...
inline void startup() {
    if (sodium_init() < 0) error("libsodium initialization failed");
}
#endif
