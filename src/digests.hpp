...
public:
    ...
    auto restart() {
        safe::memset(&out_, 0, sizeof(out_));
        finished_ = !reinit();
        return !finished_;
    }
...
#if defined(BUSUTO_CRYPTO_WOLFSSL)
...
#elif defined(BUSUTO_CRYPTO_MINICRYPT)
...
#elif defined(BUSUTO_CRYPTO_OPENSSL)
...
namespace busuto {
template <typename B>
requires digest::is_streambuf_v<B>
