...
template <std::size_t Bits, busuto::util::readable_binary Binary>
...
struct hash_index final {
    auto operator()(const T& key) const {
        if constexpr (busuto::util::readable_binary<T>) {
            return reduce_digest<sizeof(std::size_t) * 8>(key);
        } else {
            static_assert(std::is_trivially_copyable_v<T>, "hash_index requires trivially copyable T");

            const auto span = busuto::byte_span(reinterpret_cast<const std::byte *>(&key), sizeof(T));
            return hash_reduce<sizeof(std::size_t) * 8>(span);
        }
    }
};
