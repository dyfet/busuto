...
Busuto is a template library for enterprise C++ server development. It in some
ways replaces moderncli. Where it differs from ModernCLI is in core practices
and in supporting evolving C++ standards and practices much more quickly, such
as C++20. While ModernCLI and things like Boost all backport many things for
use with C++17, Busuto looks to boost into more current C++ compiler standards
directly.
...
greatly simplify the codebase by eliminating support for Microft Windows, a
server platform nobody would ever as a practical matter use for most kinds of
large scale commercial deployments. While ModernCLI supported an extensive
posix compatibility layer, this too can be safely jetisoned since Busuto only
supports real posix platforms. There are some useful posix features we can use
in C++ that are not possible to make windows portable. This also aligns busuto
more closely with a new HPX specific cousin for developing highly scalable
parallel and distributed super-computing clustered services, hitycho.

## Dependencies

All dependencies are really header based to avoid hard runtime linking
